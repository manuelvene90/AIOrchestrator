...
internal sealed class LaunchWitness_Fake(IOrchestrationLauncher inner, ISupervisionPaths paths) : IOrchestrationLauncher
{
    ...
    public bool Respawn_Supervisor(string orchId) => inner.Respawn_Supervisor(orchId);
    public void Respawn_Communicator(string orchId) => inner.Respawn_Communicator(orchId);
    public bool Respawn_Implementer(string orchId, string memberId) => inner.Respawn_Implementer(orchId, memberId);
    public bool Spawn_GeneralSupervisor() => inner.Spawn_GeneralSupervisor();
}
