...
public interface IPendingAnnouncement
{
    ...
    /// <summary>
    /// Who the entry is for. It rides in the queue because it is a property of the ENTRY, not of
    /// the moment it is written: the call site decided it, and a queued announcement is written
    /// on a later tick by a drain that knows nothing about why it was queued. Inferring it there
    /// would put the decision back where it cannot be made.
    /// </summary>
    Channels.AppEntryAudiences Audience { get; }
...
public interface IPendingAnnouncements
{
    ...
    /// </summary>
    IPendingAnnouncement? Queue(string orchId, string channelFile, Channels.AppEntryAudiences audience, string subject, string body, DateTime nowUtc);
