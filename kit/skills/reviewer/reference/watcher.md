Arm the one your `runners.reviewer.wake` names; if you do not know, arm the watcher-mode script.
...
# digest, and no way to tell the supervisor's note from your own last report.
#
...
`<orch-id>/<member-id>` for a reviewer, so `.../$ARGUMENTS/.wake` would read identically — this uses
the two env vars instead only so the same script shape serves every role command, including the ones
where `$ARGUMENTS` does not carry a member id at all (solo, the general supervisor, the communicator).

When it wakes you: read your channel, act, append your report, end your turn — exactly as in watcher
